...
import { selectPackageListByQuery, queryOrderByUseId, getMinChatLinkCountsServiceIp } from '../network/api'
var websocket = null
...
export const InitWebSocket = () => {
    ...
    const getQueryOrderByUseId = async (carParkUseLog) => {
        ...
        console.log('获取账单信息',data);
        console.log(data.cfCarParkUseLog);
    ...
    const updateInterface = async (message) => {
        ...
        if (message.cfUserMessage.type == 1) {
            ...
            if(carParkUseLog.carParkId == store.getters.carParkId){
                const direction = carParkUseLog.outTime == 0 ? " 车牌进场：" : "车牌出场：";
                if (store.state.globalConfig.promptEntryExitCar) {
                    ElNotification({
                        title: '提示',
                        message: carParkUseLog.cfCarParkCheckpointIn.name + direction + carParkUseLog.numberPlate,
    ...
    if (window.WebSocket) {
        getMinChatLinkCountsServiceIp().then(res => {
            ...
            websocket = new ReconnectingWebSocket(`ws://${res.data}:9002/ws`);
            websocket.onopen = (e) => {
