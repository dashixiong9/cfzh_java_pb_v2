<template>
	<view>
		<view class="addCarBox u-flex u-row-center" @click="intoAddCar">
			<image src="../../../static/image/my/Add@2x.png" ></image>
			<view class="text">添加车辆</view>
		</view>
		<view v-if="list.length == 0" class="u-flex-c u-col-center u-row-center">
			<image src="../../../static/image/home/nocar.png" class="list_not"></image>
			<view class="no_title1">您还未绑定车牌哦</view>
			<view class="no_title2">请输入您需要购买的车牌</view>
		</view>
		<view v-else>
			<view class="u-flex-col">
				<view class="carBox u-flex-col u-rela" v-for="(item,index) in list" :key="index">
					...
					<image src="../../../static/image/my/delete@2x.png"  @click="deleteCar(item)"></image>
				</view>
			</view>
			<!-- <view class="u-flex-c">
				...
				uni.showModal({
					...
					success: async showModalres => {
						if(showModalres.cancel){
							var res = await this.$u.api.deleteCarparkcar(item.id)
