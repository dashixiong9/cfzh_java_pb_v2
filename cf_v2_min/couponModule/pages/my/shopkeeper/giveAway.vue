<template>
	<view>
		...
		<view class="list" v-if="0 == current">
			<view class="li" style="border-top: 20rpx solid #f7f7f7;">
				...
				<picker mode="selector" :range="amountList2" @change="amountChange">
					<input class="right" disabled v-model="amount" type="number" placeholder="请选择金额(元)"
		...
		<view class="list" v-if="1 == current">
			<view class="li" style="border-top: 20rpx solid #f7f7f7;">
				<view class="left">小时数</view>
				<input class="right" v-model="time" type="number" placeholder="请输入小时数"
					:style="plateNo?'color: #323233;text-align:right':'color: rgba(50, 50, 51, 0.5);text-align:right'"></input>
			</view>
			<view class="li" style="border-top: 20rpx solid #f7f7f7;">
				<view class="left">手机号</view>
	...
	export default {
		...
		methods: {
			...
			async staffOrShopkeeperGiveCouponToCar(value = '') {
				...
				var res = await this.$u.api.staffOrShopkeeperGiveCouponToCar({
					quantity,
					couponType,
					numberPlate: this.plateNo,
					phone: this.phone ? this.phone : '',
					cfCouponActivityId: this.id,
					couponActivityCouponTypeId: this.couponActivityTypeId || ''
				})
			...
			intoCode() {
				...
				const params = {
					...
					sourceType,
					quantity,
					couponActivityTypeId: this.couponActivityTypeId
				}
				uni.navigateTo({
