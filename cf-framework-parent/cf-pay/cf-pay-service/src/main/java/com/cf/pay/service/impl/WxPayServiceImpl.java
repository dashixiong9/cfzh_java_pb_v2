...
public class WxPayServiceImpl implements WxPayService {
    ...
    public ResultMap unifiedOrder(String orderNo, double amount, String body, String ip, String openid, CfUserPaymentAgency cfUserPaymentAgency) throws Exception{
        ...
        if(StringUtils.isNotEmpty(wxPayAppConfig.getSubAppID())){
            requestMap.put("sub_appid", wxPayAppConfig.getSubAppID());
        }
        requestMap.put("mch_id", wxPayAppConfig.getMchID());
        if(StringUtils.isNotEmpty(wxPayAppConfig.getSubMchID())){
            requestMap.put("sub_mch_id", wxPayAppConfig.getSubMchID());
        }
        requestMap.put("nonce_str", WXPayUtil.generateNonceStr());
        if(StringUtils.isNotEmpty(openid)){
            requestMap.put("openid", openid);                                     // 用户openid
        }
        long timeMillis = System.currentTimeMillis();
        requestMap.put("time_start", DateUtil.stampToDate(timeMillis, "yyyyMMddHHmmss"));                                     // 订单生成时间
        requestMap.put("time_expire", DateUtil.stampToDate(timeMillis+180000L, "yyyyMMddHHmmss"));                                     // 订单失效时间
        ...
        if(cfUserPaymentAgency.getPaymentAgencyShortName().equals("wei_xin_app_pay_cny")){
            requestMap.put("trade_type", "APP");
        }else{
            requestMap.put("trade_type", "JSAPI");
        }                          // App支付类型
        requestMap.put("notify_url", wxPayAppConfig.getPayNotifyUrl());   // 接收微信支付异步通知回调地址  wxPayAppConfig.getPayNotifyUrl()
        requestMap.put("sign", WXPayUtil.generateSignature(requestMap, wxPayAppConfig.getKey(), WXPayConstants.SignType.MD5));//微信支付签名
        ...
        if ((!returnMsg.equals("OK") && (responseMap == null || responseMap.isEmpty())) || resultMap.get("result_code").equals("FAIL")) {
            String errorMsg = null;
            if(StringUtils.isNotEmpty(resultMap.get("err_code_des"))){
                errorMsg = resultMap.get("err_code_des");
            }else if(StringUtils.isNotEmpty(resultMap.get("return_msg"))){
                errorMsg = resultMap.get("return_msg");
            }
            logger.error("pay_error_desc:"+errorMsg);
            ExceptionCast.cast(PayCode.FAILED_TO_OBTAIN_PREPAID_TRANSACTION_SESSION_ID, errorMsg);
        }
        // 3、签名生成算法
