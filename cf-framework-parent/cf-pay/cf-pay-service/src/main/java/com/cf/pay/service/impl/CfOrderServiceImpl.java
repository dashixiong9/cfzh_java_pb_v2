        ...
        switch (cfUserPaymentAgency.getPaymentAgencyShortName()) {
            case "wei_xin_pay_cny":
                ...
                if(cfUserPaymentAgency.getAccountNumberNote().equals("yi_sheng_pay")){
                    ...
                    body.put("authId",openid);    //用户openid
                    body.put("customerNum",cfUserPaymentAgency.getAppid());    //商户编号
                    body.put("agentNum",cfUserPaymentAgency.getSubAppid()); //代理编号
                    body.put("shopNum",cfUserPaymentAgency.getMchId());    //店铺编号
                    body.put("bankType","WX_XCX");    //支付方式
                    body.put("requestNum",cfOrder.getId());   //商户订单号
                    body.put("amount",String.valueOf(cfOrder.getAmountsPayable().doubleValue()));  //金额
                    body.put("callbackUrl", cfUserPaymentAgency.getPayNotifyUrl());   //回调地址

                    long currentTimeMillis = System.currentTimeMillis();
                    String token = BCryptUtil.SHA1("secretKey="+cfUserPaymentAgency.getSecret()+"&timestamp="+currentTimeMillis+
                            "&path=/v1/customer/order/pay/create&body="+JSONObject.toJSONString(body)).toUpperCase();

                    heard.put("accessKey", cfUserPaymentAgency.getPayKey());
                    heard.put("timestamp", currentTimeMillis+"");
                    heard.put("token", token);

                    Connection.Response result = HttpUtils.post("https://openapi.duolabao.com/v1/customer/order/pay/create",heard, JSONObject.toJSONString(body), HttpUtils.JSON_TYPE);
                    Map jsonObject = JSONObject.parseObject(result.body());
                    if(!jsonObject.get("result").toString().equals("success")){
                        System.out.println("提交的参数:"+JSONObject.toJSONString(body));
                        System.out.println("请求头:"+JSONObject.toJSONString(heard));
                        System.out.println("接口返回:"+jsonObject);
                        ExceptionCast.cast(PayCode.WECHAT_PAY_FAILED_TO_PLACE_A_UNIFIED_ORDER, jsonObject.get("error").toString());
                    }
                    Map<String, String> bankRequest = (Map<String, String>) ((Map) jsonObject.get("data")).get("bankRequest");
                    Map<String, String> returnMap = new HashMap<>();
                    returnMap.put("appId",bankRequest.get("APPID"));
                    returnMap.put("timeStamp",bankRequest.get("TIMESTAMP"));
                    returnMap.put("nonceStr",bankRequest.get("NONCESTR"));
                    returnMap.put("package",bankRequest.get("PACKAGE"));
                    returnMap.put("signType",bankRequest.get("SIGNTYPE"));
                    returnMap.put("sign",bankRequest.get("PAYSIGN"));
            ...
            case "ali_pay_cny":
                ...
                if(cfUserPaymentAgency.getAccountNumberNote().equals("duo_la_bao")){
                    ...
                    response.put("out_trade_no",bankRequest.get("OUT_TRADE_NO"));
                    response.put("trade_no",bankRequest.get("TRADENO"));

                    returnMap.put("alipay_trade_create_response",response);
                    resultMap = ResultMap.ok().put("data", returnMap);
                }else{
...
//                    Object alipayConfig = alipayService.initAlipayConfig(cfUserPaymentAgency);
                    try {
                        resultMap = alipayService.doPay(cfOrder, cfThirdPartyLogins.get(0).getOpenid(), cfUserPaymentAgency);
                    } catch (Exception e) {
                        if(e.getMessage().equals("交易已存在且与本次请求的订单金额不一致") || e.getMessage().equals("交易买家不匹配")){
                            //如果是重复支付，给其再重新生成最新的id
                            String newOrderId = idWorker.nextId();
                            flushOrderId(newOrderId, cfOrder.getId());
                            cfOrder.setId(newOrderId);
                            resultMap = alipayService.doPay(cfOrder, cfThirdPartyLogins.get(0).getOpenid(), cfUserPaymentAgency);
                        }
                    }
                }
            ...
            case "ali_h5_pay_cny":
                if(!cfUserPaymentAgency.getAccountNumberNote().equals("yi_sheng_pay")) {
                    resultMap = alipayService.doPay(cfOrder, "wap",cfUserPaymentAgency);
                }else{
                    //易生支付
                    openid = "";
                    cfThirdPartyLoginQuery = new CfThirdPartyLoginQuery();
                    cfThirdPartyLoginQuery.setUid(cfOrder.getUid());
                    cfThirdPartyLoginQuery.setAppid(cfUserPaymentAgency.getAppid());
                    cfThirdPartyLoginQuery.setPlatform("ali_web");
                    cfThirdPartyLogins = cfThirdPartyLoginService.getListByQuery(cfThirdPartyLoginQuery);
                    if(cfThirdPartyLogins.size()>0){
                        openid = cfThirdPartyLogins.get(0).getOpenid();
                    }
