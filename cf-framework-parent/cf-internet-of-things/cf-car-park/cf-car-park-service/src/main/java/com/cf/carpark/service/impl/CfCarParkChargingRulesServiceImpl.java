...
//        }
    ...
    public CfCarParkOrder calculateTheAmounPayable(CfCarParkUseLog cfCarParkUseLog, String uid, String queryMode) throws Exception {
        ...
        if(cfCarParkOrder.getCfOrder()!=null && (cfCarParkOrder.getCfOrder().getPayTime()>0L || cfCarParkOrder.getCfOrder().getStatus()!=(byte)0)){
    ...
    private CfCarParkOrder calculateTheAmounPayableWithOutOther(CfCarParkUseLog cfCarParkUseLog, String uid) throws Exception{
        ...
        if(cfOrder!=null && (cfOrder.getPayTime()>0L || cfOrder.getStatus()!=(byte)0)){
...
//                if((cfCarParkUseLog.getOutTime() - cfCarParkUseLog.getInTime()>cfCarPark.getFreeTime()) && orderList!=null && orderList.size()>0 && previousCarParkUseLog!=null && (((cfCarParkUseLog.getOutTime()-previousCarParkUseLog.getInTime())<=86400000l) || ((cfCarParkUseLog.getOutTime()-previousCarParkUseLog.getInTime())>86400000l && (previousCarParkUseLog.getOutTime()-previousCarParkUseLog.getInTime())>86400000l && (cfCarParkUseLog.getOutTime()-previousCarParkUseLog.getOutTime())<=86400000l))){
    ...
    public Double fixedCalculateTheAmounPayable(Long startTime, Long endTime, CfCarPark cfCarPark, Double unitPrice, String currentCalculateingCarType, CfCarPark calculateingCfCarPark, Integer counts, Long upperLimitTime) throws Exception
    {
        ...
        if(counts==0 && cfCarPark.getStartingPrice().doubleValue()>0){
            totalFee = cfCarPark.getStartingPrice().doubleValue();
            startTime += cfCarPark.getUnitDuration();
        }
        counts++;
        if(endTime<=startTime){
            return totalFee;
        ...
        if(cfCarPark.getFeeUpperLimit().doubleValue()>0) {
            if (endTime <= DateUtil.maxMillisecondBaseOnTheDayToTimestamp(startTime)) {
                totalFee += Math.ceil((double)(endTime - startTime) / cfCarPark.getUnitDuration()) * unitPrice;
                if (totalFee >= cfCarPark.getFeeUpperLimit().doubleValue()) {
                    totalFee = cfCarPark.getFeeUpperLimit().doubleValue();
//                    startTime = upperLimitTime.longValue()>0 ? startTime+upperLimitTime : DateUtil.maxMillisecondBaseOnTheDayToTimestamp(startTime) + 1001;
//                    totalFee += fixedCalculateTheAmounPayable(startTime, DateUtil.maxMillisecondBaseOnTheDayToTimestamp(startTime) + 1001, cfCarPark, unitPrice, currentCalculateingCarType, calculateingCfCarPark, counts, upperLimitTime);
                }
            } else {
                if(DateUtil.maxMillisecondBaseOnTheDayToTimestamp(startTime)>startTime){
                    totalFee += Math.ceil((double)(DateUtil.maxMillisecondBaseOnTheDayToTimestamp(startTime) - startTime) / cfCarPark.getUnitDuration()) * unitPrice;
                    if (totalFee >= cfCarPark.getFeeUpperLimit().doubleValue()) {
                        totalFee = cfCarPark.getFeeUpperLimit().doubleValue();
                    }
                    startTime = upperLimitTime.longValue()>0 ? startTime+upperLimitTime : DateUtil.maxMillisecondBaseOnTheDayToTimestamp(startTime) + 1001;
                    totalFee += fixedCalculateTheAmounPayable(startTime, endTime, cfCarPark, unitPrice, currentCalculateingCarType, calculateingCfCarPark, counts, upperLimitTime);
                }
            }
        } else {
            totalFee += Math.ceil((double)(endTime - startTime) / cfCarPark.getUnitDuration()) * unitPrice;
    ...
    public Double fixed24HoursCalculateTheAmounPayable(Long startTime, Long endTime, CfCarPark cfCarPark, Double unitPrice, String currentCalculateingCarType, CfCarPark calculateingCfCarPark, Integer counts) throws Exception {
        ...
        if(cfCarPark.getFeeUpperLimit().doubleValue()>0) {
            //停车超过24小时
            if ((counts==1 && cfCarPark.getStartingPrice().doubleValue()>0) && (endTime-tempStartTime>86400000)){
                totalFee = cfCarPark.getFeeUpperLimit().doubleValue();
                startTime += 86400000-cfCarPark.getUnitDuration();
                totalFee += fixed24HoursCalculateTheAmounPayable(startTime, endTime, cfCarPark, unitPrice, currentCalculateingCarType, calculateingCfCarPark, counts);
            }else if (endTime-startTime>86400000){
                totalFee = cfCarPark.getFeeUpperLimit().doubleValue();
                startTime += 86400000;
                totalFee += fixed24HoursCalculateTheAmounPayable(startTime, endTime, cfCarPark, unitPrice, currentCalculateingCarType, calculateingCfCarPark, counts);
            }else{
                double totalFeeTemp = Math.ceil((double)(endTime - startTime) / cfCarPark.getUnitDuration()) * unitPrice;
                if(totalFeeTemp+totalFee>=cfCarPark.getFeeUpperLimit().doubleValue()){
                    totalFee = cfCarPark.getFeeUpperLimit().doubleValue();
                }else{
    ...
    public Double dynamicCalculateTheAmounPayable(Long startTime, Long endTime, Double totalFee, String carType, CfCarPark calculateingCfCarPark, Integer counts,
        ...
        if(counts==0 && calculateingCfCarPark.getStartingPrice().doubleValue()>0){
            totalFee = calculateingCfCarPark.getStartingPrice().doubleValue();
            startTime += calculateingCfCarPark.getUnitDuration();
        }
        if(endTime<=startTime){
            return  totalFee;
        }
        counts++;
        if(carParkChargingRules==null){
            CfCarParkChargingRulesQuery cfCarParkChargingRulesQuery = new CfCarParkChargingRulesQuery();
            cfCarParkChargingRulesQuery.setStatus((byte)1);
            cfCarParkChargingRulesQuery.setCarType(carType);
            cfCarParkChargingRulesQuery.setCarParkId(calculateingCfCarPark.getId());
            carParkChargingRules = getListByCondition(cfCarParkChargingRulesQuery);
            if(carParkChargingRules==null || carParkChargingRules.size()==0){
                return totalFee;
            }
        ...
        Long time1 = 0l;
        Long time2 = 0l;
        Double currentDayFee = 0d;
        boolean crossDays = false;
        //只停一天
        if(DateUtil.minMillisecondBaseOnTheDayToTimestamp(startTime).longValue()==DateUtil.minMillisecondBaseOnTheDayToTimestamp(endTime).longValue()){
            time1 = startTime-DateUtil.minMillisecondBaseOnTheDayToTimestamp(startTime);
            time2 = endTime-DateUtil.minMillisecondBaseOnTheDayToTimestamp(endTime);
            crossDays = false;
        }else{
            //跨天停车
            time1 = startTime-DateUtil.minMillisecondBaseOnTheDayToTimestamp(startTime);
        ...
        //判断是否有混合收费模式
        String billingModel = "";
        for(CfCarParkChargingRules cfCarParkChargingRule: carParkChargingRules){

            if(time1<cfCarParkChargingRule.getEndTime() && cfCarParkChargingRule.getFeeUpperLimit().doubleValue()>0 && !cfCarParkChargingRule.getGroupFlag().equals(calculateingCfCarPark.getCurrentGroupFlag())){
                calculateingCfCarPark.setCurrentFeeUpperLimit(cfCarParkChargingRule.getFeeUpperLimit());
                calculateingCfCarPark.setCurrentGroupFlag(cfCarParkChargingRule.getGroupFlag());
                totalFee += currentDayFee;
                currentDayFee = 0d;
                countFee = 0d;

                calculateingCfCarPark.setFeeUpperLimit(new BigDecimal(0));
            }

            if(cfCarParkChargingRule.getFeeUpperLimit().doubleValue()>0 && currentDayFee.doubleValue()>=cfCarParkChargingRule.getFeeUpperLimit().doubleValue()){
                time1 = cfCarParkChargingRule.getEndTime();
                continue;
            }

            if(time1>=cfCarParkChargingRule.getEndTime()){
                continue;
            }else if(time2>cfCarParkChargingRule.getEndTime()) {
                if(cfCarParkChargingRule.getType().equals(CfCarParkChargingRulesServiceImpl.EACH)){
                    currentDayFee = cfCarParkChargingRule.getFee().doubleValue();
                }else if(cfCarParkChargingRule.getType().equals(CfCarParkChargingRulesServiceImpl.EACH_ADD)){
                    currentDayFee += cfCarParkChargingRule.getFee().doubleValue();
                }else{
                    currentDayFee += Math.ceil((double) (cfCarParkChargingRule.getEndTime() - time1) / calculateingCfCarPark.getUnitDuration()) * (cfCarParkChargingRule.getFee().doubleValue());
                }
                time1 = cfCarParkChargingRule.getEndTime();
                if(cfCarParkChargingRule.getFeeUpperLimit().doubleValue()==0){
                    if(upperLimitTime.doubleValue()>0 && calculateingCfCarPark.getFeeUpperLimit().doubleValue()>0 && (currentDayFee+countFee)>=calculateingCfCarPark.getFeeUpperLimit().doubleValue()){
                        totalFee += calculateingCfCarPark.getFeeUpperLimit().doubleValue();
                        countFee = 0d;
                        startTime = originalStartTime+upperLimitTime+1001;
                        totalFee = dynamicCalculateTheAmounPayable(startTime,endTime,totalFee,carType,calculateingCfCarPark,counts,carParkChargingRules, upperLimitTime, countFee, startTime);
                        return totalFee;
                    }
                    currentDayFee += countFee;
                }else if(cfCarParkChargingRule.getFeeUpperLimit().doubleValue()>0){
                    if((currentDayFee+countFee)>=cfCarParkChargingRule.getFeeUpperLimit().doubleValue()){
                        currentDayFee = cfCarParkChargingRule.getFeeUpperLimit().doubleValue();
                    }
                }
                ...
                if(cfCarParkChargingRule.getType().equals(CfCarParkChargingRulesServiceImpl.EACH)){
                    ...
                    currentDayFee += Math.ceil((double) (time2 - time1) / calculateingCfCarPark.getUnitDuration()) * (cfCarParkChargingRule.getFee().doubleValue());
                }
                crossDays = false;
                if(cfCarParkChargingRule.getFeeUpperLimit().doubleValue()==0 && upperLimitTime.doubleValue()>0 && calculateingCfCarPark.getFeeUpperLimit().doubleValue()>0 && (currentDayFee+countFee)>=calculateingCfCarPark.getFeeUpperLimit().doubleValue()){
                    totalFee += calculateingCfCarPark.getFeeUpperLimit().doubleValue();
                    countFee = 0d;
                    startTime = originalStartTime+upperLimitTime+1001;
                    totalFee = dynamicCalculateTheAmounPayable(startTime,endTime,totalFee,carType,calculateingCfCarPark,counts,carParkChargingRules, upperLimitTime, countFee, startTime);
                    return totalFee;
                }else if(cfCarParkChargingRule.getFeeUpperLimit().doubleValue()==0 && upperLimitTime.doubleValue()>0 && calculateingCfCarPark.getFeeUpperLimit().doubleValue()>0 && (currentDayFee+countFee)<calculateingCfCarPark.getFeeUpperLimit().doubleValue()){
                    currentDayFee += countFee;
                }else if(cfCarParkChargingRule.getFeeUpperLimit().doubleValue()>0){
                    if((currentDayFee+countFee)>=cfCarParkChargingRule.getFeeUpperLimit().doubleValue()){
                        currentDayFee = cfCarParkChargingRule.getFeeUpperLimit().doubleValue();
        ...
        if(calculateingCfCarPark.getFeeUpperLimit().doubleValue()>0 && currentDayFee>=calculateingCfCarPark.getFeeUpperLimit().doubleValue()){
            currentDayFee = calculateingCfCarPark.getFeeUpperLimit().doubleValue();
        }

        if(crossDays){
            if(calculateingCfCarPark.getFeeUpperLimit().doubleValue()>0 && currentDayFee>=calculateingCfCarPark.getFeeUpperLimit().doubleValue()
                    && upperLimitTime.doubleValue()>0){
                startTime = originalStartTime+upperLimitTime+1001;
            }else if(calculateingCfCarPark.getFeeUpperLimit().doubleValue()>0 && currentDayFee<calculateingCfCarPark.getFeeUpperLimit().doubleValue()
                    && upperLimitTime.doubleValue()>0){
                countFee = currentDayFee;
                originalStartTime = startTime;
                startTime = DateUtil.maxMillisecondBaseOnTheDayToTimestamp(startTime)+1001;
                currentDayFee = 0d;
            }else{
                startTime = DateUtil.maxMillisecondBaseOnTheDayToTimestamp(startTime)+1001;
                countFee = currentDayFee;
            }
            if(calculateingCfCarPark.getCurrentFeeUpperLimit().doubleValue()>0){
                currentDayFee = 0d;
            }
            totalFee = dynamicCalculateTheAmounPayable(startTime,endTime,totalFee,carType,calculateingCfCarPark,counts,carParkChargingRules, upperLimitTime, countFee, originalStartTime);
        }

        return totalFee+currentDayFee;
    }
    ...
    public Double dynamic24HoursCalculateTheAmounPayable(Long totalTime, Long startCountTime, String carType, CfCarPark calculateingCfCarPark, Integer counts, List<CfCarParkChargingRules> carParkChargingRules, double currentPrice) throws Exception {
        ...
        long currentStartCountTime = startCountTime;
        if(startCountTime>=totalTime){
            return 0d;
        }
        if(counts==0 && calculateingCfCarPark.getStartingPrice().doubleValue()>0){
            totalFee = calculateingCfCarPark.getStartingPrice().doubleValue();
            startCountTime += calculateingCfCarPark.getUnitDuration();
        }
        if(startCountTime>=totalTime){
            return totalFee;
        }
        ...
        for(CfCarParkChargingRules cfCarParkChargingRules: carParkChargingRules){
            if(totalTime<cfCarParkChargingRules.getStartTime()){
                break;
            }
            //次数计费
            if(cfCarParkChargingRules.getType().equals(CfCarParkChargingRulesServiceImpl.EACH)){
                if(currentPrice==cfCarParkChargingRules.getFee().doubleValue()){
                    continue;
                }else{
                    currentPrice = cfCarParkChargingRules.getFee().doubleValue();
                    totalFee = currentPrice;
                }
            } else if(cfCarParkChargingRules.getType().equals(CfCarParkChargingRulesServiceImpl.EACH_ADD)){
                if(currentPrice==cfCarParkChargingRules.getFee().doubleValue()){
                    continue;
                }else{
                    currentPrice = cfCarParkChargingRules.getFee().doubleValue();
            ...
            //时段计费
                ...
                if(totalTime>=cfCarParkChargingRules.getEndTime()){
                    totalFee += Math.ceil((double)(cfCarParkChargingRules.getEndTime() - cfCarParkChargingRules.getStartTime()) / calculateingCfCarPark.getUnitDuration()) * cfCarParkChargingRules.getFee().doubleValue();
                }else{
                    totalFee += Math.ceil((double)(totalTime - cfCarParkChargingRules.getStartTime()) / calculateingCfCarPark.getUnitDuration()) * cfCarParkChargingRules.getFee().doubleValue();
                }
            }
            if(calculateingCfCarPark.getFeeUpperLimit().doubleValue()>0 && totalFee>calculateingCfCarPark.getFeeUpperLimit().doubleValue()){
                totalFee = calculateingCfCarPark.getFeeUpperLimit().doubleValue();
            }
        }
        //跨24小时
        if(totalTime-currentStartCountTime>=86400000l) {
            totalFee += dynamic24HoursCalculateTheAmounPayable(totalTime - 86400000l, 0l, carType, calculateingCfCarPark, counts, carParkChargingRules, 0);
        }
        return totalFee;
    }
