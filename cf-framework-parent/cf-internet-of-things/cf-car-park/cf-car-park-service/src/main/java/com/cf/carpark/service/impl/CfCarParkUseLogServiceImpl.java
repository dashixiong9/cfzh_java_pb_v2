...
//        forbidRepeatLog(cfCarParkUseLog);
    ...
    public CfCarParkUseLog checkIsZhenShi911202002050(Map carLogMap) throws Exception
    {
        ...
        if( StringUtils.isNotEmpty(zhenShi911202002050.getAlarmInfoPlate().getResult().getPlateResult().getLicense())){
            if(carLogMap.containsKey("use_base64")){
                String license = zhenShi911202002050.getAlarmInfoPlate().getResult().getPlateResult().getLicense();
                cfCarParkUseLog.setNumberPlate(new String(Base64.getDecoder().decode(license)));
            }else{
                cfCarParkUseLog.setNumberPlate(zhenShi911202002050.getAlarmInfoPlate().getResult().getPlateResult().getLicense());
            }
            cfCarParkUseLog.setCarNumberPlateColor(zhenShi911202002050.getAlarmInfoPlate().getResult().getPlateResult().getColorType().byteValue());
//            if(zhenShi911202002050.getAlarmInfoPlate().getResult().getPlateResult().getCar_brand()!=null){
//                cfCarParkUseLog.setCarBrand(zhenShi911202002050.getAlarmInfoPlate().getResult().getPlateResult().getCar_brand().getBrand());
//            }
            ...
            }else if(StringUtils.isNotEmpty(zhenShi911202002050.getAlarmInfoPlate().getResult().getPlateResult().getImagePath())){
                if(carLogMap.containsKey("use_base64")){
                    smallImage = StringTools.parseURL(new String(Base64.getDecoder().decode(zhenShi911202002050.getAlarmInfoPlate().getResult().getPlateResult().getImageFragmentPath())));
                }else{
                    smallImage = zhenShi911202002050.getAlarmInfoPlate().getResult().getPlateResult().getImagePath().replace(".","_plate.");
                }
            }
            if(StringUtils.isNotEmpty(zhenShi911202002050.getAlarmInfoPlate().getResult().getPlateResult().getImageFile())){
                ...
                if(carLogMap.containsKey("use_base64")){
                    bigImage = StringTools.parseURL(new String(Base64.getDecoder().decode(zhenShi911202002050.getAlarmInfoPlate().getResult().getPlateResult().getImagePath())));
                }else{
                    bigImage = zhenShi911202002050.getAlarmInfoPlate().getResult().getPlateResult().getImagePath();
                }
            }
        }else{
            return null;
        ...
        if(carLogMap.containsKey("cfCarParkDeviceId")){
            ...
            cfCarParkDevice = cfCarParkDeviceService.findByCode(zhenShi911202002050.getAlarmInfoPlate().getSerialno(), false);
        }
        ...
        CfCarParkUseLog carParkUseLog = checkExistsSubscribeLog(cfCarParkUseLog);
        if(carParkUseLog!=null){
            return carParkUseLog;
        }
        Long inOrOutTime = System.currentTimeMillis();
        if(zhenShi911202002050.getAlarmInfoPlate().getResult().getPlateResult().getTimeStamp()!=null){
    ...
    public CfCarParkUseLog checkIsHuaXia(Map carLogMap) throws Exception {
        ...
        cfCarParkUseLog.setParkingSpaceNumber(huaXia.getBerthcode());
        String smallImage = "";
        String bigImage = "";
        if( StringUtils.isNotEmpty(huaXia.getCar_plate()) || StringUtils.isNotEmpty(huaXia.getPlate_num()) ){
            ...
            if(StringUtils.isNotEmpty(huaXia.getSmallImagePath())){
                smallImage = huaXia.getSmallImagePath();
            }
            if(StringUtils.isNotEmpty(huaXia.getBigImagePath())){
                smallImage = huaXia.getBigImagePath();
            }
            if(carLogMap.containsKey("bigImagePath") && StringUtils.isNotEmpty(carLogMap.get("bigImagePath").toString())){
                bigImage = carLogMap.get("bigImagePath").toString();
            }
        }else{
            return null;
        }
        ...
        //获取相机
        String barCode = StringUtils.isNotEmpty(huaXia.getCamera_id()) ? huaXia.getCamera_id() : huaXia.getCam_id();
        ...
        Long inOrOutTime = (Integer.valueOf(huaXia.getStart_time()))*1000L;
        return handleUseLogByDevicePushData(cfCarParkUseLog, cfCarParkDevice, inOrOutTime, smallImage, bigImage);
    }
    ...
    public Object parseCarParkUseLogForm(Map carLogMap) throws Exception{
        ...
        if(deviceBrand.equals(DeviceBrand.ZHEN_SHI) || deviceBrand.equals(DeviceBrand.QIAN_YI)){
            ...
            if(carLogMap.containsKey("AlarmInfoPlate")){
                ...
                jsonObject.put("name", "serial_data");
                jsonObject.put("version", "1.0");
                jsonObject.put("timestamp", System.currentTimeMillis()/1000);
                JSONObject payload = new JSONObject();
                payload.put("type","serial_data");
                JSONObject body = new JSONObject();
                body.put("serialData",zhenShiResponse911202002050.getResponse_AlarmInfoPlate().getSerialData());
                payload.put("body",body);
                jsonObject.put("payload", payload);
                zhenShiResponse911202002050.getResponse_AlarmInfoPlate().setIs_pay(jsonObject.toString());
    ...
    public Object handleCameraData(CfCarParkUseLog cfCarParkUseLog, Map carLogMap, String cameraBrand) throws Exception {
        ...
        //判断是否为无牌车
        if(cfCarParkUseLog.getNumberPlate().equals("无牌车")){
            CfCarParkCarLimitQuery cfCarParkCarLimitQuery = new CfCarParkCarLimitQuery();
            cfCarParkCarLimitQuery.setCarTypeKey("unlicensed_car");
            ...
            if(cfCarParkCarLimits!=null && cfCarParkCarLimits.size()>0){
                cfCarParkCarLimit = cfCarParkCarLimits.get(0);
            }
        }else{
            cfCarParkCarLimit = cfCarParkCarLimitService.getCarTypeLimit(cfCarParkUseLog.getNumberPlate(), cfCarParkUseLog.getCarParkId());
        }

        CfCarPark cfCarPark = cfCarParkService.findById(cfCarParkUseLog.getCarParkId(), false);
        cfCarParkUseLog.setCfCarPark(cfCarPark);

        cfCarParkUseLog.setCarType("temporary_car");
        if(cfCarParkCarLimit!=null){
            //设置车辆类型
            cfCarParkUseLog.setCarType(cfCarParkCarLimit.getCarTypeKey());
        }else if(cfCarPark.getNumberPlateColorToCarType()==(byte)1 && cfCarParkUseLog.getCarNumberPlateColor()!=null && cfCarParkUseLog.getCarNumberPlateColor()!=(byte)0){
            CfCarParkCarLimitQuery cfCarParkCarLimitQuery = new CfCarParkCarLimitQuery();
            switch (cfCarParkUseLog.getCarNumberPlateColor()){
                case (byte)1:
                    cfCarParkCarLimitQuery.setCarTypeKey("blue_number_plate_car");  //蓝牌车
                    break;
                case (byte)2:
                    cfCarParkCarLimitQuery.setCarTypeKey("yellow_number_plate_car");    //黄牌车
                    break;
                case (byte)3:
                    cfCarParkCarLimitQuery.setCarTypeKey("white_number_plate_car"); //白牌车
                    break;
                case (byte)4:
                    cfCarParkCarLimitQuery.setCarTypeKey("block_number_plate_car"); //黑牌车
                    break;
                case (byte)5:
                    cfCarParkCarLimitQuery.setCarTypeKey("green_number_plate_car"); //绿牌车
                    break;
            }
            cfCarParkCarLimitQuery.setCarParkId(cfCarParkUseLog.getCarParkId());
            ...
            if(cfCarParkCarLimits!=null && cfCarParkCarLimits.size()>0){
                ...
                cfCarParkUseLog.setCarType(cfCarParkCarLimitQuery.getCarTypeKey());
            }
        }

        if(cfCarParkUseLog!=null){
            //将数据推送到及时通讯消息端
            CfCarParkCheckpoint cfCarParkCheckpoint = null;
            if(cfCarParkUseLog.getOutTime()==null){
                cfCarParkUseLog.setOutTime(0L);
            }
            if(cfCarParkUseLog.getOutTime()>0){
                cfCarParkCheckpoint = cfCarParkCheckpointService.findById(cfCarParkUseLog.getOutCheckPointId(), false);
                cfCarParkUseLog.setOutHandleUid(cfCarParkCheckpoint.getDutyUid());
                if(cfCarParkUseLog.getInHandleUid()==null){
                    cfCarParkUseLog.setInHandleUid("");
                }
            }else{
                cfCarParkCheckpoint = cfCarParkCheckpointService.findById(cfCarParkUseLog.getInCheckPointId(), false);
                cfCarParkUseLog.setInHandleUid(cfCarParkCheckpoint.getDutyUid());
                if(cfCarParkUseLog.getOutHandleUid()==null){
                    cfCarParkUseLog.setOutHandleUid("");
                }
            }
            ...
            Long outTime = cfCarParkUseLog.getOutTime();
            if(outTime==null){
                cfCarParkUseLog.setOutTime(0L);
            }
            if(outTime>0){
            ...
            String showCarTypeName = "临时车";
            String voiceCarTypeName = "临时车";
            ...
            if(cfCarParkPackageList!=null && cfCarParkPackageList.size()>0){
                //获取车辆类型
                CfCarParkCarType cfCarParkCarType = cfCarParkCarTypeService.findByKey(cfCarParkPackageList.get(0).getTypeKey());
                showCarTypeName = cfCarParkCarType!=null ? "月卡车":showCarTypeName;
                voiceCarTypeName = cfCarParkCarType!=null ? "月卡车":voiceCarTypeName;
                if(cfCarParkPackageList.get(0).getEndTime()<System.currentTimeMillis()){
                    showCarTypeName += "-已过期";
                    cfCarParkUseLog.setCarStatus(cfCarParkPackageList.get(0).getStatus());
                }
                cfCarParkUseLog.setCarType(cfCarParkCarType.getFlagKey());
//                cfCarParkUseLog.setVisitUnit(cfCarParkPackageList.get(0).getCarOwnerName());
            }

            PlayRule playRule = null;
            if(cameraBrand.equals(DeviceBrand.ZHEN_SHI) || cameraBrand.equals(DeviceBrand.QIAN_YI) || cameraBrand.equals(DeviceBrand.HUA_XIA) || cameraBrand.equals(DeviceBrand.HK) || cameraBrand.equals(DeviceBrand.DH)){
                playRule = SeriaDataUtils.setSerialDataZhenshiMultiple(cfCarParkUseLog.getCfCarParkDevice().getPlayContents(), SeriaDataUtils.APPROACH, cfCarParkUseLog.getNumberPlate(), "text", "plaintext");
            }else{
                ExceptionCast.cast(CarParkCode.BRAND_DEVICES_THAT_IS_NOT_CURRENTLY_SUPPORTED);
            }

            CfCarParkUseLog fixFeeCfCarParkUseLog = null;

            //进场提示
            if(cfCarParkUseLog.getInTime()>0 && cfCarParkUseLog.getOutTime()==0){
                playRule = makeCarInNotice(cfCarPark,cfCarParkUseLog,cfCarParkPackageList,cfCarParkCarLimit,cfCarParkPackageQuery,playRule,cfCarParkCheckpoint,showCarTypeName,voiceCarTypeName,cameraBrand,zhenShiResponse911202002050,huaXiaResponse);
                cfCarParkCarLimit = cfCarParkUseLog.getCfCarParkCarLimit();
            }
            ...
            cfUserMessage.setToUid(cfCarParkCheckpoint.getDutyUid());
            cfUserMessage.setGroupId("");
            if((cfCarParkCheckpoint.getAllowTemporaryCar()==(byte)0 && showCarTypeName.equals("临时车")) || (cfCarParkCheckpoint.getAllowExpiredCar()==(byte)0 && cfCarParkPackageList.size()>0 && cfCarParkPackageList.get(0).getEndTime()<=System.currentTimeMillis())
                    || (cfCarParkCheckpoint.getAllowPackageCar()==(byte)0 && cfCarParkPackageList.size()>0 && cfCarParkPackageList.get(0).getEndTime()>System.currentTimeMillis())){
                cfCarParkUseLog.setReleaseNotice("该通道已禁止该类车辆通行");
            ...
            //进场提示
            else if(cfCarParkUseLog.getInTime()>0 && cfCarParkUseLog.getOutTime()==0){
                cfCarParkUseLog.setReleaseNotice(cfCarParkUseLog.getNumberPlate()+" 车辆进场");
                if(cameraBrand.equals(DeviceBrand.ZHEN_SHI) || cameraBrand.equals(DeviceBrand.QIAN_YI)){
                ...
                String onOrOff = "off";
                String outShow = "祝您一路顺风";
                if(cfCarPark.getAbnormalAutoRelease()==(byte)0){
                    outShow = "请稍等";
                    if(cfCarParkUseLog.getNumberPlate().length()>=6){
                        //匹配可能车辆并推送给岗亭人员
                        ...
                        cfCarParkUseLogQuery.setOutTime(0l);
                        cfCarParkUseLogQuery.setMinInTime(System.currentTimeMillis()-172800000);
                        List<CfCarParkUseLog> cfCarParkUseLogs = new ArrayList<>();
                        //车位进行位移
                        for(int i=0; i<=1; i++){
                            cfCarParkUseLogQuery.setNumberPlateLK(cfCarParkUseLog.getNumberPlate().substring(i+2,i+(cfCarParkUseLog.getNumberPlate().length()-2)));
                            List<CfCarParkUseLog> carParkUseLogList = getListByQuery(cfCarParkUseLogQuery);
                            if(carParkUseLogList!=null && carParkUseLogList.size()>0){
                                for(CfCarParkUseLog carParkUseLog: carParkUseLogList){
                                    cfCarParkUseLogs.add(carParkUseLog);
                                }
                            }
                        }
                        if(cfCarParkUseLogs!=null && cfCarParkUseLogs.size()>0){
                            ...
                            for (CfCarParkUseLog parkUseLog: cfCarParkUseLogs){
                                FileUtils.handleFileSourcePrefix(parkUseLog, fileSourceAddress, "inSmallImage");
                                FileUtils.handleFileSourcePrefix(parkUseLog, fileSourceAddress, "outSmallImage");
                                FileUtils.handleFileSourcePrefix(parkUseLog, fileSourceAddress, "inBigImage");
                                FileUtils.handleFileSourcePrefix(parkUseLog, fileSourceAddress, "outBigImage");
                            }
                            ...
                            Map<String, Object> msgContests = new HashMap<>();
                            msgContests.put("before",cfCarParkUseLogs);
                            cfCarParkUseLog.setId(newestCarParkId);
                            msgContests.put("now",cfCarParkUseLog);
                            //推送给岗亭人员
                            if(StringUtils.isNotEmpty(cfCarParkCheckpoint.getDutyUid())){
                                executeMessage(msgContests, cfCarParkCheckpoint.getDutyUid(), 6);
                            }
                            cfCarParkUseLog.setId(null);
                        }
                    }
                }else{
                    onOrOff = "on";
                ...
                playRule = SeriaDataUtils.setSerialDataZhenshiMultiple(JSONObject.toJSONString(playRule), SeriaDataUtils.ABNORMALOUT, cfCarParkUseLog.getNumberPlate(), "text", "plaintext");
                playRule = SeriaDataUtils.setSerialDataZhenshiMultiple(JSONObject.toJSONString(playRule), SeriaDataUtils.ABNORMALOUT, showCarTypeName, "text", "plaintext");
                playRule = SeriaDataUtils.setSerialDataZhenshiMultiple(JSONObject.toJSONString(playRule), SeriaDataUtils.ABNORMALOUT, "异常放行", "text", "plaintext");
                playRule = SeriaDataUtils.setSerialDataZhenshiMultiple(JSONObject.toJSONString(playRule), SeriaDataUtils.ABNORMALOUT, outShow, "text", "plaintext");

                playRule = SeriaDataUtils.setSerialDataZhenshiMultiple(JSONObject.toJSONString(playRule), SeriaDataUtils.ABNORMALOUT, cfCarParkUseLog.getNumberPlate(), "voice", "plaintext");
                playRule = SeriaDataUtils.setSerialDataZhenshiMultiple(JSONObject.toJSONString(playRule), SeriaDataUtils.ABNORMALOUT, voiceCarTypeName, "voice", "plaintext");
                playRule = SeriaDataUtils.setSerialDataZhenshiMultiple(JSONObject.toJSONString(playRule), SeriaDataUtils.ABNORMALOUT, outShow, "voice", "plaintext");

                if(cameraBrand.equals(DeviceBrand.ZHEN_SHI) || cameraBrand.equals(DeviceBrand.QIAN_YI)){
                    showByDeviceRows(playRule, cfCarParkUseLog.getCfCarParkDevice(), SeriaDataUtils.ABNORMALOUT);
                    SeriaDataUtils.setSerialDataZhenshiByPlayRule(zhenShiResponse911202002050, playRule, SeriaDataUtils.ABNORMALOUT,"");
                }else if(cameraBrand.equals(DeviceBrand.HUA_XIA)){
                    SeriaDataUtils.setSerialDataHuaXiaByPlayRule(huaXiaResponse, playRule, SeriaDataUtils.ABNORMALOUT,"");
                }else if(cameraBrand.equals(DeviceBrand.HK) || cameraBrand.equals(DeviceBrand.DH)){
                    //不做任何操作
                }else{
                    ExceptionCast.cast(CarParkCode.BRAND_DEVICES_THAT_IS_NOT_CURRENTLY_SUPPORTED);
                ...
                cfCarParkUseLog.setReleaseNotice(cfCarParkUseLog.getNumberPlate()+" 车辆 无进场记录放行");
                if(cameraBrand.equals(DeviceBrand.ZHEN_SHI) || cameraBrand.equals(DeviceBrand.QIAN_YI)){
                    setCameraOpenOrClose(zhenShiResponse911202002050, onOrOff);
                }else if(cameraBrand.equals(DeviceBrand.HUA_XIA)){
                    setCameraOpenOrClose(huaXiaResponse, onOrOff);
                }else if(cameraBrand.equals(DeviceBrand.HK) ||cameraBrand.equals(DeviceBrand.DH)){
                    hkResponse.setOpenDoor(onOrOff);
                }else{
                    ExceptionCast.cast(CarParkCode.BRAND_DEVICES_THAT_IS_NOT_CURRENTLY_SUPPORTED);
                }

            }else if(cfCarParkUseLog.getInTime()>0 && cfCarParkUseLog.getOutTime()>0){
                //车辆正常出场，判断是否有费用，如果有费用不提示
                //尝试检查和使用自动缴费(银行扣款或者无感支付)
                playRule = normalOutHandle(cfCarParkUseLog, fixFeeCfCarParkUseLog, cfCarParkCarLimit, cfOrder, cameraBrand, zhenShiResponse911202002050, huaXiaResponse, hkResponse, playRule, showCarTypeName, voiceCarTypeName, cfCarParkPackageList);
            }else{
                if(cameraBrand.equals(DeviceBrand.ZHEN_SHI) || cameraBrand.equals(DeviceBrand.QIAN_YI)){
                    setCameraOpenOrClose(zhenShiResponse911202002050, "off");
                }else if(cameraBrand.equals(DeviceBrand.HUA_XIA)){
            ...
            if(cfCarParkUseLog.getPayTime()>0){
                ...
                cfOrders.get(0).setPayTime(cfCarParkUseLog.getPayTime());
                cfOrders.get(0).setStatus(PayStatus.PAID);
                cfOrderService.update(cfOrders.get(0));
            }

            if(StringUtils.isEmpty(cfCarParkUseLog.getReleaseNotice()) && fixFeeCfCarParkUseLog==null){
                ...
                playRule = SeriaDataUtils.setSerialDataZhenshiMultiple(JSONObject.toJSONString(playRule), SeriaDataUtils.PAYOUT, DateUtil.getDifferenceByTimeStamp(cfCarParkUseLog.getInTime(),cfCarParkUseLog.getOutTime()), "text", "duration");
            ...
            if(cameraBrand.equals(DeviceBrand.ZHEN_SHI) || cameraBrand.equals(DeviceBrand.QIAN_YI)){
                zhenShiResponse911202002050.getResponse_AlarmInfoPlate().setPlateid(cfCarParkUseLog.getId());
            }

            if((cfCarPark.getLimitParkingSpaceNumber()==(byte)1 && cfCarPark.getStatus()==(byte)1) || (cfCarParkCarLimit!=null && cfCarParkCarLimit.getForbidIn()==(byte)1) || cfCarParkCheckpoint.getStatus()==(byte)1 || (cfCarParkCheckpoint.getAllowTemporaryCar()==(byte)0 && showCarTypeName.equals("临时车")) || (cfCarParkCheckpoint.getAllowExpiredCar()==(byte)0 && cfCarParkPackageList.size()>0 && cfCarParkPackageList.get(0).getEndTime()<=System.currentTimeMillis())) {
                if(cfCarParkUseLog.getOutTime()==0){

                    //预约到访车辆判断
                    boolean visitStatus = false;
                    if(cfCarPark.getVisitProcessMode().byteValue()>(byte)0){
                        //判断车场开启了预约到访功能，查询该车辆是否有预约
                        CfCarParkVisitQuery cfCarParkVisitQuery = new CfCarParkVisitQuery();
                        cfCarParkVisitQuery.setCarParkId(new Long(cfCarParkUseLog.getCarParkId()));
                        cfCarParkVisitQuery.setPlateNumber(cfCarParkUseLog.getNumberPlate());
                        cfCarParkVisitQuery.setProcessStatus((byte)1);
                        CfCarParkVisit updateCarParkVisit = new CfCarParkVisit();
                        updateCarParkVisit.setProcessStatus((byte)2);
                        Integer updateByQuery = cfCarParkVisitService.updateByQuery(updateCarParkVisit, cfCarParkVisitQuery);
                        if(updateByQuery.intValue()>0){
                            visitStatus = true;
                        }
                    }
                    if(!visitStatus){
                ...
                //如果状态为关闭状态，和禁止通行车辆类型，不播报语音
                if(cfCarParkCheckpoint.getStatus()==(byte)1 || (cfCarParkCarLimit!=null && cfCarParkCarLimit.getForbidIn()==(byte)1)){
                    if(cameraBrand.equals(DeviceBrand.ZHEN_SHI) || cameraBrand.equals(DeviceBrand.QIAN_YI)){
                        setCameraOpenOrClose(zhenShiResponse911202002050, "off");
//                        zhenShiResponse911202002050.getResponse_AlarmInfoPlate().setSerialData(null);
                    }else if(cameraBrand.equals(DeviceBrand.HUA_XIA)){
                        setCameraOpenOrClose(huaXiaResponse, "off");
//                        huaXiaResponse.setRs485_data(null);
                    }else if(cameraBrand.equals(DeviceBrand.HK) || cameraBrand.equals(DeviceBrand.DH)){
                        hkResponse.setOpenDoor("off");
//                        hkResponse.setPlayRule(null);
                    }else{
                        ExceptionCast.cast(CarParkCode.BRAND_DEVICES_THAT_IS_NOT_CURRENTLY_SUPPORTED);
                    }
                }
            }else if(cfCarParkUseLog.getOutTime()==0 && cfCarParkCheckpoint.getAllowExpiredCar()==(byte)1 && cfCarParkPackageList!=null && cfCarParkPackageList.size()>0 && cfCarParkPackageList.get(0).getEndTime()<=System.currentTimeMillis()){
                //允许过期车辆自动入场
                if(cameraBrand.equals(DeviceBrand.ZHEN_SHI) || cameraBrand.equals(DeviceBrand.QIAN_YI)){
                    setCameraOpenOrClose(zhenShiResponse911202002050, "on");
                }else if(cameraBrand.equals(DeviceBrand.HUA_XIA)){
            ...
            //产生费用的套餐车辆禁止出场
            else if(cfCarParkUseLog.getOutTime()>0 && cfCarParkPackageList!=null && cfCarParkPackageList.size()>0 && cfOrder!=null && cfOrder.getAmountsPayable()!=null && cfOrder.getAmountsPayable().doubleValue()>0){
                if(cameraBrand.equals(DeviceBrand.ZHEN_SHI) || cameraBrand.equals(DeviceBrand.QIAN_YI)){
                    setCameraOpenOrClose(zhenShiResponse911202002050, "off");
                }else if(cameraBrand.equals(DeviceBrand.HUA_XIA)){
                    setCameraOpenOrClose(huaXiaResponse, "off");
            ...
            //判断特殊车辆是否自动入场开闸
            if(cfCarParkUseLog.getOutTime()==0 && (cfCarParkCheckpoint.getStatus()==(byte)1 || (cfCarParkCheckpoint.getStatus()==(byte)0 && cfCarParkCarLimit!=null))){
                String onOrOff = null;
                if(cfCarParkCarLimit==null){
                    onOrOff = cfCarParkCheckpoint.getStatus()==(byte)1 ? "off" : "on";
                }else{
                    onOrOff = cfCarParkCarLimit.getAutoOpenDoor()==(byte)0 ? "off" : "on";
                }
                if(cameraBrand.equals(DeviceBrand.ZHEN_SHI) || cameraBrand.equals(DeviceBrand.QIAN_YI)){
                    setCameraOpenOrClose(zhenShiResponse911202002050, onOrOff);
                }else if(cameraBrand.equals(DeviceBrand.HUA_XIA)){
            ...
            if(cfCarParkCheckpoint.getStatus()==(byte)0 && cfOrder!=null && StringUtils.isNotEmpty(cfOrder.getCollectionShopId())){
                if(cameraBrand.equals(DeviceBrand.ZHEN_SHI) || cameraBrand.equals(DeviceBrand.QIAN_YI)){
                    setCameraOpenOrClose(zhenShiResponse911202002050, "on");
                }else if(cameraBrand.equals(DeviceBrand.HUA_XIA)){
                    setCameraOpenOrClose(huaXiaResponse, "on");
            ...
            if(!(cfCarParkCheckpoint.getAddForbiddenData()==(byte)0 && ((cfCarParkCheckpoint.getAllowTemporaryCar()==(byte)0 && showCarTypeName.equals("临时车")) || (cfCarParkCheckpoint.getAllowExpiredCar()==(byte)0 && cfCarParkPackageList.size()>0 && cfCarParkPackageList.get(0).getEndTime()<=System.currentTimeMillis())))
                    && !(cfCarParkCarLimit!=null && cfCarParkCarLimit.getForbidIn()==(byte)1) && cfCarPark.getStatus()==(byte)0){
                if(cfCarParkUseLog.getOutTime()>0 && cfCarParkUseLog.getPayTime()>0){
                    cfCarParkUseLog.setOutReleaseType(ReleaseType.AUTO);
                }
                if((zhenShiResponse911202002050.getResponse_AlarmInfoPlate().getInfo().equals("ok") || huaXiaResponse.getGpio_data().get(0).equals("on") || hkResponse.getOpenDoor().equals("on")) && cfCarParkUseLog.getInTime()>0 && cfCarParkUseLog.getOutTime()==0){
                    cfCarParkUseLog.setInReleaseType(ReleaseType.AUTO);
                }
                if(StringUtils.isNotEmpty(cfCarParkUseLog.getId())){
                    cfCarParkUseLog = update(cfCarParkUseLog);
                }else{
                    cfCarParkUseLog.setId(newestCarParkId);
                ...
                }else if((zhenShiResponse911202002050.getResponse_AlarmInfoPlate().getInfo().equals("ok") || huaXiaResponse.getGpio_data().get(0).equals("on") || hkResponse.getOpenDoor().equals("on")) && cfCarParkUseLog.getOutTime()>0){
                    cfCarParkService.updateUsedParkingSpaceNumber(cfCarParkUseLog.getCarParkId(),-1);
                }
            }

            //如果该停车场允许下发白名单，且当前车牌刚好是状态正常的，且是非家庭组的，禁止下发开闸命令，避免与本地白名单冲突，造成重复开闸命令多次重叠
//            if(cfCarPark.getAutoIssuedWhitelist()==(byte)1 && (zhenShiResponse911202002050.getResponse_AlarmInfoPlate().getInfo().equals("ok") || huaXiaResponse.getGpio_data().get(0).equals("on") || hkResponse.getOpenDoor().equals("on")) && cfCarParkPackageList.size()>0 && cfCarParkPackageList.get(0).getEndTime()>=System.currentTimeMillis() && StringUtils.isEmpty(cfCarParkPackageList.get(0).getGroupFlag())){
//                if(cameraBrand.equals(DeviceBrand.ZHEN_SHI) || cameraBrand.equals(DeviceBrand.QIAN_YI)){
//                    setCameraOpenOrClose(zhenShiResponse911202002050, "off");
//                }else if(cameraBrand.equals(DeviceBrand.HUA_XIA)){
//                    setCameraOpenOrClose(huaXiaResponse, "off");
//                }else if(cameraBrand.equals(DeviceBrand.HK)){
//                    hkResponse.setOpenDoor("off");
//                }else{
//                    ExceptionCast.cast(CarParkCode.BRAND_DEVICES_THAT_IS_NOT_CURRENTLY_SUPPORTED);
//                }
//            }
            ...
            if(StringUtils.isNotEmpty(cfCarParkCheckpoint.getDutyUid())){
                if(fixFeeCfCarParkUseLog==null){
                    sendMessage(cfCarParkUseLog, cfCarParkCheckpoint.getDutyUid(), 1);
                }else{
                    if(cfCarParkUseLog!=null){
                        fixFeeCfCarParkUseLog.setOutCheckPointId(cfCarParkUseLog.getOutCheckPointId());
                        fixFeeCfCarParkUseLog.setOutHandleUid(cfCarParkUseLog.getOutHandleUid());
                    }
                    CfCarParkUseLog parkUseLog = new CfCarParkUseLog();
                    parkUseLog.setId(fixFeeCfCarParkUseLog.getId());
                    parkUseLog.setOutCheckPointId(cfCarParkUseLog.getOutCheckPointId());
                    parkUseLog.setOutHandleUid(cfCarParkUseLog.getOutHandleUid());
                    updateByPrimaryKeySelective(parkUseLog);
                    sendMessage(fixFeeCfCarParkUseLog, cfCarParkCheckpoint.getDutyUid(), 1);
                }
            }
            //通知超级管理员 大屏数据
                ...
                try {
                    ...
                    if(cfUserRoleList!=null && cfUserRoleList.size()>0){
                        for (CfUserRole cfUserRole: cfUserRoleList){
                            cfCarParkUseLog.setCfCarPark(cfCarPark);
                            sendMessage(cfCarParkUseLog, cfUserRole.getUid(), 1);
                        }
                    }
                } catch (Exception e) {
                    e.printStackTrace();
            ...
            //推送LCD屏幕消息
            if(cfCarParkUseLog.getCfCarParkDevice()!=null && StringUtils.isNotEmpty(cfCarParkUseLog.getCfCarParkDevice().getMacAddress())){
                Map<String, TextPlay> textPlay = null;
                if(playRule.getApproach().getTextPlay()!=null && playRule.getApproach().getTextPlay().size()>0){
                    Map<String, Object> stringObjectMap = (Map<String, Object>) JSONObject.toJSON(playRule.getApproach().getTextPlay().get("1"));
                    int status = Integer.parseInt(stringObjectMap.get("status").toString());
                    if(status==1){
                        textPlay = playRule.getApproach().getTextPlay();
                    }
                }
                if(playRule.getAbnormalOut().getTextPlay()!=null && playRule.getAbnormalOut().getTextPlay().size()>0){
                    Map<String, Object> stringObjectMap = (Map<String, Object>) JSONObject.toJSON(playRule.getAbnormalOut().getTextPlay().get("1"));
                    int status = Integer.parseInt(stringObjectMap.get("status").toString());
                    if(status==1){
                        textPlay = playRule.getAbnormalOut().getTextPlay();
                    }
                }
                if(playRule.getFreeOut().getTextPlay()!=null && playRule.getFreeOut().getTextPlay().size()>0){
                    Map<String, Object> stringObjectMap = (Map<String, Object>) JSONObject.toJSON(playRule.getFreeOut().getTextPlay().get("1"));
                    int status = Integer.parseInt(stringObjectMap.get("status").toString());
                    if(status==1){
                        textPlay = playRule.getFreeOut().getTextPlay();
                    }
                }
                if(playRule.getPayOut().getTextPlay()!=null && playRule.getPayOut().getTextPlay().size()>0){
                    Map<String, Object> stringObjectMap = (Map<String, Object>) JSONObject.toJSON(playRule.getPayOut().getTextPlay().get("1"));
                    int status = Integer.parseInt(stringObjectMap.get("status").toString());
                    if(status==1){
                        textPlay = playRule.getPayOut().getTextPlay();
                    }
                }
                sendLcdMessage(cfCarParkUseLog.getCfCarParkDevice().getId(),3, JSONObject.toJSONString(textPlay));
            }
            ...
            if(cameraBrand.equals(DeviceBrand.ZHEN_SHI) || cameraBrand.equals(DeviceBrand.QIAN_YI)){
                ...
                hkResponse.setPlayRule(playRule);
                return hkResponse;
            }else{
                ExceptionCast.cast(CarParkCode.BRAND_DEVICES_THAT_IS_NOT_CURRENTLY_SUPPORTED);
            }
...
//                "LEFT JOIN cf_payment_agency pa ON(ord.payment_agency_short_name=pa.short_name) ";
    ...
    public CfCarParkUseLog applyForAccess(String uid, String direction, String checkPointId, byte type, String deviceNo) throws Exception {
        ...
        if(direction.equals(CarDirection.IN)){
            ...
            }else if(cfCarParkOrder.getCfOrder()!=null && StringUtils.isNotEmpty(cfCarParkOrder.getCfOrder().getId())){
                List<CfCoupon> cfCouponList = cfCouponService.getAvailableByShopIdAndScenes(System.currentTimeMillis(), cfCarParkOrder.getCfOrder().getShopId(), CouponScenes.CARPARK, cfCarParkOrder.getCfOrder().getEffectObject(), uid);
                //如果人工设置了优惠金额，电子优惠券不再使用
                if(cfCouponList!=null && cfCouponList.size()>0 && (cfCarParkOrder.getCfOrder().getManualOfferSetTime()==0 || (System.currentTimeMillis()-cfCarParkOrder.getCfOrder().getManualOfferSetTime()>=900000))){
                    CfCoupon cfCoupon = cfCouponList.get(0);
                    ...
                    if((cfCoupon.getCouponType()==(byte)1 && cfCoupon.getDenomination().intValue()>0) ||
                        ...
                        cfCarParkOrder.getCfCarParkUseLog().setPayTime(System.currentTimeMillis());
                        carParkUseLog.setPayTime(cfCarParkOrder.getCfCarParkUseLog().getPayTime());
                        //结束其对应的账单
                        CfOrder updateOrder = new CfOrder();
                        updateOrder.setId(cfCarParkOrder.getCfOrder().getId());
                        updateOrder.setAmountActuallyPaid(cfCarParkOrder.getCfOrder().getAmountsPayable());
                        updateOrder.setPaymentAgencyShortName("coupon");
                        updateOrder.setStatus((byte)1);
                        updateOrder.setPayTime(carParkUseLog.getPayTime());
                        cfOrderService.updateByPrimaryKeySelective(updateOrder);
                        //将对应的优惠券使用掉
                        CfCoupon updateCoupon = new CfCoupon();
                        updateCoupon.setId(cfCoupon.getId());
                        updateCoupon.setStatus((byte)0);
                        updateCoupon.setUseTime(updateOrder.getPayTime());
                        updateCoupon.setAmountUsed(cfCarParkOrder.getCfOrder().getAmountsPayable());
                        cfCouponService.updateByPrimaryKeySelective(updateCoupon);
                    }else{
                        return carParkUseLog;
                    }
                }
            ...
            carParkUseLog.setOutTime(System.currentTimeMillis());
            cfCarParkUseLog.setOutReleaseType((byte)1);
            carParkUseLog.setOutReleaseType((byte)1);
    ...
    public void lVGEMSmartConstructionPlatform(CfCarParkUseLog cfCarParkUseLog, Map<String, String> config) throws Exception {
        ...
        if(weixinConfigs==null || weixinConfigs.size()==0){
            ...
            JSONObject result = (JSONObject)HttpClient.doPost(params, "http://202.105.146.194:16007/authority/api/getAuthenticate", header, true);
