...
public class DeviceChannelMap {
    // 用户保存用户id与通道的Map对象
    private static Map<String, CfGroundLockClient> userChannelMap;

    static {
        userChannelMap = new HashMap<String, CfGroundLockClient>();
    }
     ...
     */
    public static void put(String userid, CfGroundLockClient cfChargingClient) {
     ...
     * @return Netty通道
     */
    public static CfGroundLockClient get(String friendid) {
        return userChannelMap.get(friendid);
    }

    public static void printf(){
        for ( Map.Entry<String, CfGroundLockClient> entry: userChannelMap.entrySet()){
