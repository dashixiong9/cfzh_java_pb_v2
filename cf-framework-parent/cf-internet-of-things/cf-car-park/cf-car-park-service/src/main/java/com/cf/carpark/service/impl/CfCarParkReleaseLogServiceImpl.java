...
public class CfCarParkReleaseLogServiceImpl implements CfCarParkReleaseLogService, ApplicationContextAware {
    ...
    public CfCarParkReleaseLog add(CfCarParkReleaseLog cfCarParkReleaseLog, String redisPreFix) throws Exception {
        ...
        if(cfCarParkReleaseLog.getCfCarParkDevice()!=null && cfCarParkReleaseLog.getCfCarParkDevice().getLinkMode().equals("mqtt") && cfCarParkReleaseLog.getCfCarParkDevice().getType().byteValue()==(byte)2){
            int channel = redisPreFix.equals(REDIS_PRE) ? 0 : 1;
            Object o = executeRelease(cfCarParkReleaseLog, cfCarParkReleaseLog.getCfCarParkDevice().getBrand(), channel);
...
//                mqttClient.startMQTTClient(cfCarParkReleaseLog.getCfCarParkDevice().getBarCode(), JSON.toJSONString(o));
    ...
    public String getDeviceLinkLocalServerIp(String sn) throws Exception{
        String deviceLinkIp = redisTemplate.boundValueOps(MqttHandler.PARKING_DEVICE_LINK_IP + sn).get();
        ...
        if(InetAddress.getLocalHost().getHostAddress().equals(deviceLinkIp)){
            ...
            MqttHandler.offlineDevice.put(sn, System.currentTimeMillis());  //标记掉线设备
                ...
                if(cfCarParkReleaseLog.getCfCarParkUseLog()!=null && cfCarParkReleaseLog.getCfCarParkUseLog().getCfOrder()!=null && cfCarParkReleaseLog.getCfCarParkUseLog().getCfOrder().getStatus()==PayStatus.PAID && cfCarParkReleaseLog.getCfCarParkUseLog().getCfOrder().getAmountActuallyPaid().doubleValue()>0){
                    try {
                        ...
                        if(playRule!=null){
                            ...
                            ZhenShiResponse911202002050 zhenShiResponse911202002050_485 = null;
                            zhenShiResponse911202002050_485 = SeriaDataUtils.setSerialDataZhenshiByPlayRule(zhenShiResponse911202002050_485, playRule, SeriaDataUtils.FREEOUT,"");
                            //进行485数据下发
                            JSONObject jsonObject485 = new JSONObject();
                            jsonObject485.put("id", StringTools.getRandomString(null,16));
                            jsonObject485.put("sn", cfCarParkReleaseLog.getCfCarParkDevice().getBarCode());
                            jsonObject485.put("name", "serial_data");
                            jsonObject485.put("version", "1.0");

                            jsonObject485.put("timestamp", System.currentTimeMillis()/1000);
                            JSONObject payload485 = new JSONObject();
                            payload485.put("type","serial_data");
                            JSONObject body485 = new JSONObject();
                            body485.put("serialData",zhenShiResponse911202002050_485.getResponse_AlarmInfoPlate().getSerialData());
                            payload485.put("body",body485);
                            jsonObject485.put("payload", payload485);
                            zhenShiResponse911202002050_485.getResponse_AlarmInfoPlate().setIs_pay(jsonObject485.toString());
                            iMqttDataService.pushTopics(cfCarParkReleaseLog.getCfCarParkDevice().getBarCode(), JSON.toJSONString(zhenShiResponse911202002050_485),"/device/" + cfCarParkReleaseLog.getCfCarParkDevice().getBarCode() + "/command");
                        }
                    } catch (Exception e){
                        e.printStackTrace();
                    }
