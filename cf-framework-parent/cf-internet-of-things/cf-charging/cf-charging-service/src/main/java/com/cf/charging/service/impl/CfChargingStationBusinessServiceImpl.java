...
import com.cf.charging.service.handle.EchoClientHandler;
import com.cf.charging.service.handle.LvChongChongHandle;
import com.cf.charging.service.socket.CfChargingClient;
import com.cf.charging.service.socket.DeviceChannelMap;
...
public class CfChargingStationBusinessServiceImpl implements CfChargingStationBusinessService, ApplicationContextAware {
    ...
    @Autowired
    private LvChongChongHandle lvChongChongHandle;
    @Autowired
    ...
    public void handleLink(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        CfChargingClient cfChargingClient = null;
        //判断充电站品牌
        if(bytesToString16.indexOf("7e5d7d7f")==0){ //江西驴充充 充电设备
            cfChargingClient = lvChongChongHandle.getCfChargingClient(ctx, bytesToString16);
            CfChargingClient chargingClient = DeviceChannelMap.get(cfChargingClient.getDeviceId());
            if(cfChargingClient.getCommand().equals("0001") || chargingClient==null){
                DeviceChannelMap.put(cfChargingClient.getDeviceId(), cfChargingClient);
                //将设备在线设备ip记录到redis中去
                redisTemplate.boundValueOps(CfChargingStationBusinessServiceImpl.DEVICE_LINK_IP+cfChargingClient.getDeviceId()).set(InetAddress.getLocalHost().getHostAddress());
                redisTemplate.expire(CfChargingStationBusinessServiceImpl.DEVICE_LINK_IP+cfChargingClient.getDeviceId(),365, TimeUnit.DAYS);
            }

        }else{
            System.out.println("指令无法识别："+bytesToString16);
//            ExceptionCast.cast(ChargingCode.BRAND_DEVICES_THAT_IS_NOT_CURRENTLY_SUPPORTED);
    ...
    public CfChargingUseLog startCharging(CfChargingStationDeviceForm cfChargingStationDeviceForm) throws Exception {
        ...
        CfChargingClient cfChargingClient = DeviceChannelMap.get(cfChargingStationDeviceForm.getBarCode());
        ...
        cfChargingUseLog.setChargingType(cfChargingClient.getDeviceType());
...
//        cfChargingPortService.update(cfChargingPort);
        ...
        switch (cfChargingStationDeviceForm.getBrand()){
            case "lv_chong_chong":
                lvChongChongHandle.startCharging(cfChargingClient, cfChargingStationDeviceForm.getChargingPort().toString(), chargingAmount, cfChargingUseLog.getId());
                break;
            default:
                ExceptionCast.cast(ChargingCode.BRAND_DEVICES_THAT_IS_NOT_CURRENTLY_SUPPORTED);
        }
    ...
    public CfChargingState stopCharging(CfChargingStationDeviceForm cfChargingStationDeviceForm) throws Exception {
        ...
        switch (cfChargingStationDeviceForm.getBrand()){
            case "lv_chong_chong":
                lvChongChongHandle.stopCharging(cfChargingClient, cfChargingStationDeviceForm.getChargingPort().toString());
                break;
            default:
                ExceptionCast.cast(ChargingCode.BRAND_DEVICES_THAT_IS_NOT_CURRENTLY_SUPPORTED);
        }
    ...
    public CfChargingState chargingSettiong(CfChargingStationDeviceForm cfChargingStationDeviceForm) throws Exception {
        ...
        switch (cfChargingStationDeviceForm.getBrand()){
            case "lv_chong_chong":
                lvChongChongHandle.chargingSettiong(cfChargingClient, cfChargingStationDeviceForm.getCfChargingDeviceSetting());
                break;
            default:
                ExceptionCast.cast(ChargingCode.BRAND_DEVICES_THAT_IS_NOT_CURRENTLY_SUPPORTED);
        }
    ...
    public CfChargingState getPortRealTimeData(CfChargingStationDeviceForm cfChargingStationDeviceForm) throws Exception {
        ...
        switch (cfChargingStationDeviceForm.getBrand()){
            case "lv_chong_chong":
                lvChongChongHandle.getPortRealTimeData(cfChargingClient, cfChargingStationDeviceForm.getChargingPort().toString());
                break;
            default:
                ExceptionCast.cast(ChargingCode.BRAND_DEVICES_THAT_IS_NOT_CURRENTLY_SUPPORTED);
        }
    ...
    public String checkSameServer(String serialNumber) throws Exception {
        CfChargingClient cfChargingClient = DeviceChannelMap.get(serialNumber);
        if(cfChargingClient!=null){
            //说明该设备连接在本机
            return null;
        }
